                    ...
                    .then((result) => {
                        if (result.length === 0) {
                            ...
                            result.forEach((item, index) => {
                                hours += `<a href="${Routing.generate('booking_recap', { id: calendarEl.getAttribute('service'), date: info.dateStr, hour: result[index] })}" class="btn btn-pink btn-hours">${result[index]}</a>`;
                            });
